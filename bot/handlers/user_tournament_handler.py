...
async def handler(message: Message, command: CommandObject, state: FSMContext):
    ...
    if not player.tournament_predictions or not [tournament_prediction for tournament_prediction in player.tournament_predictions if tournament_prediction.tournament_id == tournament.id]:
