...
from bot.utils.points_results import (calculate_prediction_results,
                                      create_reset_points_obj,
                                      player_points_calculation, reset_points)
...
from bot.utils.utils_match import (create_match, create_match_prediction,
                                   ...
                                   validate_tour_date)
from bot.utils.utils_tournament import (create_tour_for_tournament,
                                        eleminated_to_front,
                                        get_all_tournaments, get_tournament)
from bot.utils.utils_user_player import (eleminate_player,
...
async def create_tournament_handler(callback_query: CallbackQuery, state: FSMContext):
    ...
    if not tournaments:
        ...
        await callback_query.message.answer(
            ...
            reply_markup=await keyboard_menu(tournament_id=tournament.id, user_id=user.id),
        )
...
async def get_results(message: Message, state: FSMContext):
    ...
    if result:
        ...
        if groups:
            ...
            results = await show_distribution(
                groups.group_distribution, tournament.players
            )
            await message.answer(
                results,
                reply_markup=await keyboard_menu(tournament_id=tournament.id, user_id=data["user_id"])
...
async def get_date(message: Message, state: FSMContext):
    await message.answer(
        'Введите дату начала тура в формате "2024-06-11 18:00:00" без кавычек:'
    )
...
async def validate_date(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            f"Дата тура {tour_date} установлена. Теперь нажмите кнопку для ввода матча тура.",
            reply_markup=webapp_keyboard(),
        )
    except ValueError:
        await message.answer(
            'Неправильный формат даты. Пожалуйста, введите дату в формате "2024-06-11 18:00:00" без кавычек.'
        )


@router.message(
    lambda message: message.web_app_data, StateFilter(TournamentMenu.webapp_matches)
)
async def set_matches(web_app_message: Message, state: FSMContext):
    ...
    tournament = await get_tournament(data["tournament_id"])
    await create_match_prediction(match, tournament)
    await web_app_message.answer(
        "Нажмите далее, если закончили", reply_markup=inline_keyboard_next
    )
...
async def set_null(callback_query: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    tournament: Tournament = data["tournament_id"]
    tournament = await get_tournament(data["tournament_id"])
...
async def input_players_to_eliminate(message: Message, state: FSMContext):
    ...
    await message.answer(
        'Если всех ввели, то нажмите "Далее', reply_markup=inline_keyboard_next
    )
...
async def process_callback_next_button(
    ...
    await eleminate_player(tournament, users_to_eliminate)
...
async def receive_prediction(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(
            "Нажмите далее, если закончили вводить результаты",
            reply_markup=inline_keyboard_next,
        )
    except ValueError:
...
async def process_callback_next_button(
    ...
    await callback_query.message.delete()
    await callback_query.message.answer(
        "Результаты на матчи тура успешно заполнены",
        reply_markup=await keyboard_menu(tournament_id=data["tournament_id"], user_id=data["user_id"])
    )
